...
#: 检测框高度换算成"字形高度"的经验系数。
#:
#: :func:`estimate_line_height` 已经用低分位数挑出了"单行框"，因此这里基本按 1.0
#: 处理（框高 ≈ 该行文字高度）。
#:
#: 早先取 0.6，是拿"表格里跨行框被合并"的样本标定的 —— 那只在表格上成立。
#: 手机截图这类行距宽松的内容，框高本身就接近字形高度，乘 0.6 会**系统性低估**，
#: 于是把明明识别得很好的图也报成"字号偏小"：实测用户一张 1264×2800 的截图
#: 结果完全正确，界面上却挂着一条吓人的警告。**误报会消耗用户对警告的信任**，
#: 所以只在真正危险时才响。
GLYPH_FROM_BOX_RATIO = 1.0
#: 触发"字号偏小、结果可能不准"提醒的门槛（缩放后的等效文字高度，像素）。
#:
#: 取 CJK 字形的可辨识下限。刻意不比 :data:`MIN_READABLE_LINE_PX` 宽松：
#: 两边的代价不对称 —— 误报只是多一句提示，漏报则是静默交付编造的数据。
MIN_RELIABLE_GLYPH_PX = 12.0
# 相邻切片的默认重叠比例（相对切片边长）
